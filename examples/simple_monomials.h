...
std::vector<std::function<Number( std::vector<Number> )>> order3Monomials() {
	// Matrix([[x], [y], [x**2*y], [x*y**2], [x**4*y], [x**3]])
	std::vector<std::function<Number( std::vector<Number> )>> result;
	result.push_back( []( const std::vector<Number>& in ) {Number res = pow( in[0], 2 ) * in[1] - in[2] + 0*in[3] + 0*in[4] + 0*in[5];
		return res; } );
	result.push_back( []( const std::vector<Number>& in ) {Number res = pow( in[1], 2 ) * in[0] - in[3] + 0*in[2] + 0*in[4] + 0*in[5];
		return res; } );
	result.push_back( []( const std::vector<Number>& in ) {Number res = pow( in[0], 4 ) * in[1] - in[4] + 0*in[2] + 0*in[3] + 0*in[5];
		return res; } );
	result.push_back( []( const std::vector<Number>& in ) {Number res = pow( in[0], 3 ) - in[5] + 0*in[1] + 0*in[2] + 0*in[3] + 0*in[4];
		return res; } );
	return result;
}
