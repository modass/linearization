 ...
 */
#include "linearization/../library.h"
	...
	try {
		...
		for ( int iX = 0; iX < NX; iX++ ) {
			for ( int iY = 0; iY < NY; iY++ ) {
				...
				// idea: create the squared error between the current point XY_val and the reference point XY_ref.
				// affine function: f(x,y) = x.cvsub * x + y.cvsub * y + cv
			}
		}
